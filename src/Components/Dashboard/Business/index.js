...
import { business } from "../../../Redux/actions/businessAction";
...
const Business = () => {
  ...
  const { businessPost } = useSelector((state) => state.business);
  useEffect(() => {
    dispatch(business());
  }, []);

  const data = businessPost.length ? (
    businessPost.map((post, i) => {
      return (
